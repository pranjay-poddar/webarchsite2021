import { Link } from "react-router-dom";
import "./Team.css"


const Team = () => {

    return (
        <><p className="team-page-heading">Our Team</p>
            <ul className="five">
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>
                <li className="transition">
                    <div className="wrapper">
                        {" "}
                        <img
                            className="more-about" />
                        <ul className="social">
                            <li className="transition">
                                <a href="https://www.facebook.com">
                                    <img src="https://cdn-icons-png.flaticon.com/128/2111/2111463.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/1051/1051326.png" />
                                </a>
                            </li>
                            <li className="transition">
                                <a href="#">
                                    <img src="https://cdn-icons-png.flaticon.com/512/145/145802.png" />
                                </a>
                            </li>
                        </ul>
                        <img src="http://dooreight.com/codepen/people/img_01.png" />
                        <h3 className="transition">
                            Lucy Copycat <em>CEO &amp; Founder</em>
                        </h3>
                        <span className="transition">
                            <div className="text-wrapper transition">
                                <p>
                                    {" "}
                                    Young artist from Barcelona. She hates Comic Sans. It was
                                    popularised in the 1960s with the release Letraset sheets
                                    containing. Various versions have evolved over the years.
                                </p>
                            </div>
                        </span>{" "}
                    </div>
                </li>




            </ul></>

    );
}

export default Team;
